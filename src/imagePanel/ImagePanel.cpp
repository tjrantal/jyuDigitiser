...
void ImagePanel::setImage(int width, int height, unsigned char* data,bool static_data, bool refresh){
	...
	if (currentImageData) {
		printf("Copy currentImageData\n");
		if (!previousImageData) {
			printf("Reserve memory for previousImageData\n");
			previousImageData = new  unsigned char[imSize.x*imSize.y * 3]; /*Reserve memory on the first copy*/
		}
		printf("Memcopy previous data\n");
		//std::memcpy(previousImageData, currentImageData, imSize.x*imSize.y * 3);	//sizeof(unsigned char)*imSize.x*imSize.y * 3);	//Copy the data
		std::copy(currentImageData, currentImageData+(imSize.x*imSize.y * 3), previousImageData);
	}
	else {
		printf("Reserve currentImageData\n");
	...
	*/
	printf("Copy data\n");
	//std::memcpy(currentImageData, data, imSize.x*imSize.y * 3);	//sizeof(unsigned char)*imSize.x*imSize.y * 3);	//Copy the data
	std::copy(data, data+(imSize.x*imSize.y * 3), currentImageData);
