...
VideoReader::VideoReader(const char* file)
{
	...
	*/
	...
	pCodecPrm = pFormatCtx->streams[videoStream]->codecpar;	//New, refers to AVCodecParameters
	width = pCodecPrm->width;
	height = pCodecPrm->height;
	...
	printf("Try to allocate codec context\n");
	//allocate codec context
	pCodecCtx = avcodec_alloc_context3(pCodec);
	printf("Try to open codec context\n");
	avcodec_parameters_to_context(pCodecCtx, pCodecPrm);
...
int VideoReader::readIndices(){
	...
	while(av_read_frame(pFormatCtx, &packet)>=0) /*Read all frames to memory*/
	{
	    ...
	    if(packet.stream_index==videoStream){		 // Is this a packet from the video stream?
			...
			//avcodec_decode_video2(pCodecCtx, tmp_picture, &frameFinished, &packet);
	        while (frameFinished == 0){	//Sent packet succesfully

				frameFinished = avcodec_receive_frame(pCodecCtx, tmp_picture);
				if (frameFinished == 0) {
					++frameNo;
...
int VideoReader::readNextFrameFromDisk(){
	...
	while(readMore ==1 && frameFinished==0) { //C(pFormatCtx, &packet)>=0
	    ...
	    if(packet.stream_index==videoStream) {		 // Is this a packet from the video stream?
			...
			frameFinished = avcodec_send_packet(pCodecCtx, &packet);
			while (frameFinished == 0) {	            // Did we get a video frame?
					frameFinished = avcodec_receive_frame(pCodecCtx, tmp_picture);
					//int64_t bestPTS = av_frame_get_best_effort_timestamp	(tmp_picture);
					if (frameFinished == 0){
